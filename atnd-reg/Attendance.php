    ...
    <form name="frmEditUser" id="frmAssignParts" method="post"
      ...
      if ((WebLib::GetVal($_POST, 'CmdAtnd') !== null) && (WebLib::GetVal($_SESSION, 'AtndDone') !== '1')) {
        if (WebLib::GetVal($_SESSION, 'FormToken') == WebLib::GetVal($_POST, 'FormToken')) {
          ...
          $MobileNo = $Data->query('Select MobileNo FROM `' . MySQL_Pre . 'Users`');
          if (count($MobileNo) > 0) {
            if (preg_match('/^\d{10}$/', $MobileNo[0]['MobileNo'])) {
              ...
              $_SESSION['Msg'] = 'Invalid Mobile No!';
            }
          }
        } else {
      ...
      if (WebLib::GetVal($_SESSION, 'AtndDone') !== '1') {
        ?>
          <span class="Notice"><b
                      style="font-size:large;">Attendance for <?php echo date('d-m-Y') . ':'; ?></b>
